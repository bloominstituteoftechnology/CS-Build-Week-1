...
function Grid () {
  ...
  randomRef.current = randomRef;

  const runningRef = useRef(running);
  runningRef.current = running;

  const speedRef = useRef(addSpeed);
  speedRef.current = speedRef;
  ...
  return (
    ...
    <h3>Generations: {generation}</h3>
      ...
      <button disabled={generation === 0 ? true : false} onClick={() => {
        runStep();
      }}>
        Next!
      </button>
      <button disabled={runningRef.current ? true : false} onClick={() => {
        setRunning(!running)
        if (!running) {
          runningRef.current = true
          runRandom()
