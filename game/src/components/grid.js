...
class Grid extends Component {
        ...
        for(let y = 0; y < this.state.grid_height; y++) {
            for(let x = 0; x < this.state.grid_widt; x++) {
                for(let i = 0; i < neighbors.length; i++) {
                    ...
                    let y1 = y + neighbor[0];
                    let x1 = x + neighbor[1];
                    if(x1 >= 0 && this.state.grid_width && y1 >= 0 && y1 < this.state.grid_height && this.state.grid[y1][x1]) {
                        counter++;
                    }
                }
    ...
    render() { 
        return ( 
            <div className="Grid" 
                style={{width: (this.state.cell_size * this.state.grid_width), height:(this.state.cell_size * this.state.grid_height)}}>
            </div>
         );
    }
}
